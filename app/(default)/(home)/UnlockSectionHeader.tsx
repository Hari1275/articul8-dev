...
interface UnlockSectionHeaderProps {
  data: {
    PrefixTItle: string;
    DynamicWords: string;
    SuffixTitle: string;
...
const UnlockSectionHeader = ({ data }: UnlockSectionHeaderProps) => {
  const words = data?.DynamicWords?.split(',').map(word => word.trim()) || 
    ['New Revenue', 'Innovation', 'Efficiency', 'Differentiation'];
      ...
      <section className='bg-[#F9FAFB] pb-8 sm:py-0 sm:pt-0'>
        <div className='container mx-auto px-4'>
          ...
          >
            ...
            <span className='block sm:inline'>{data?.SuffixTitle || 'with your data'}</span>
          </h2>
        </div>
      </section>
    </ErrorBoundary>
